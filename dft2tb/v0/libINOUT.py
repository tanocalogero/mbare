...
from tbtncTools import dagger, list2range_TBTblock, CAP, get_dft_param
...
def rearrange_H(TSHS, a_inner, HS_host, pos_dSE=None, 
    ...
    a_dSE_host, new_HS_host = map_xyz(A=TSHS, B=HS_host, center_B=pos_dSE, 
        area_Delta=area_Delta, a_Delta=a_inner, area_for_buffer=None, tol=None)
...
#    geom_inner2.write('test2.xyz')
    ...
    #for i, e in enumerate(E):
        ...
        S_oi = couplingMat(S_d, o_outer, o_inner)
        ...
        SE_i = np.dot(np.dot(dagger(V_oi-e.real*S_oi), G_o), V_oi-e.real*S_oi)
...
def in2out_frame_PBCon(TSHS, a_inner, eta_value, energies, TBT, 
    ...
    # Here we don't want to remove periodic boundary conditions
    # Use in2out_frame_PBCoff to do it. Remember that there only Gamma can be used 
    TSHS_n = TSHS.copy()
...
#    geom_inner2.write('test2.xyz')
    ...
    if kfromfile:
        ...
        # If usetrs is True, then restore TRS if absent from file
        # We leave this chance open because the user might want to 
        # use this TBTGF file in a tbtrans calculation where trs should not be used (e.g. bond-currents) 
        if (TBT.kpt < 0.).any():
            print('Time reversal symmetry can be used in file. Here usetrs is set to {}.'.format(str(usetrs)))
            mp = si.MonkhorstPack(TSHS.geom, kmesh, trs=usetrs)
            klist = mp.k
            wklist = mp.weight
        else:
            klist = TBT.kpt
            wklist = TBT.wkpt
            mp = si.BrillouinZone(TSHS_n); BZ._k = klist; BZ._w = wklist
    else:
        if kmesh is None:
    ...
    for ispin, HS4GF, k, e in GF:
        ...
        # Read H and S from full TSHS (L+D+R) - no self-energies here!
        if TSHS_n.spin.is_polarized:
            Hfullk = TSHS_n.Hk(k, format='array', spin=spin)
        else:
            Hfullk = TSHS_n.Hk(k, format='array')
        Sfullk = TSHS_n.Sk(k, format='array')
        ...
        # if there's a self energy enclosed by the frame
        if TBTSE:
            SE_ext = TBTSE.self_energy('tip', E=e.real, k=k, sort=True)
            invG_o[pv_outer, pv_outer.T] -= SE_ext
        ...
        Sigma_in_HS_host[o_dSE_host, o_dSE_host.T] = SE_i
        delta_Sigma = si.Hamiltonian.fromsp(new_HS_host.geom, Sigma_in_HS_host)
        dSE.write_delta(delta_Sigma, k=k, E=e.real)
...
def in2out_frame_PBCoff_TB_PBCon(TSHS, a_inner, eta_value, energies, TBT, 
    ...
    # Here we remove periodic boundary conditions
    TSHS_n = TSHS.copy()
    print('Removing periodic boundary conditions')
    TSHS_n.set_nsc([1]*3)
