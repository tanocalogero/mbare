...
def map_xyz(A, B, center_B, area_Delta, area_int_for_buffer=None):
    ...
    if np.allclose(SE_xyz_inTSHS - v1[None,:], SE_xyz_inTB - v2[None,:]):
...
def out2in_frame(TSHS, a_inner, eta_value, energies, TBT, 
    HS_host, pzidx=None, pos_dSE=0, TSHS_elec=None, dx=None, dy=None, TBTSE=None):
    """
...
####### INPUT FILES
dir = '/zhome/6d/5/111711/DTU-SCRATCH/dft2tb/inout_tip/hpc/z1.8/gate0/'
...
#    WW=ww, LL=ll, elec=elec, save=False, return_bands=False)
HS_dev = makeTB(TSHS_0, pzidx=2, nn='all', 
    WW=ww, LL=ll, elec=elec, save=False, return_bands=False)
# Try with complete interpolation method by Nick...check email

# Define frame
a_Delta, a_int, Delta, area_int = Delta(TSHS, HS_dev, shape='Cuboid', 
